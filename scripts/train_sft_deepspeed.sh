...
DS_CONFIG="${2:-ds_config/zero2.json}"
...
if command -v llamafactory-cli >/dev/null 2>&1; then
  ...
  deepspeed --num_gpus="${GPU_COUNT}" llamafactory-cli train "${CONFIG_PATH}" --deepspeed "${DS_CONFIG}"
else
  echo "[train_sft_deepspeed] llamafactory-cli not found. Falling back to ${LLAMA_FACTORY_DIR}/src/train.py"
  deepspeed --num_gpus="${GPU_COUNT}" python "${LLAMA_FACTORY_DIR}/src/train.py" "${CONFIG_PATH}" --deepspeed "${DS_CONFIG}"
fi
